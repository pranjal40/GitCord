#include<stdio.h>
//to calculate  final amount including tax for a item at a time
float calculator(float p,float r,int q)
{float f=(p+(p*r))*q;//function to calculate  amount
return f;
}
int main()
{
...
printf("the final price for given item:%f",final_amount);//output
